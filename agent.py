...
class RLCoupledAgent(BaseAgent):
    def __init__(self, env, state_size, history_len=1, load_filepath=None):
        super(RLCoupledAgent, self).__init__(env, load_filepath)
        self.Qpolicy = Policy(state_size=state_size,action_dim=4)
        self.Qtarget = Policy(state_size=state_size,action_dim=4)
        self.Vpolicy = Policy(state_size=state_size,action_dim=1)
        self.Vtarget = Policy(state_size=state_size,action_dim=1)
        ...
        for param in self.Qtarget.parameters():
            param.requires_grad = False
        for param in self.Vtarget.parameters():
            param.requires_grad = False
    ...
    def next_move(self, epsilon=0.05):
        ...
        context = Variable(torch.from_numpy(state), requires_grad=False)
        self.prev_states.append(self.create_current_frame())
        qs = self.Qpolicy(context)
        vs = self.Vpolicy(context)
        self.prev_q = qs.data
        self.prev_v = vs.data
        # Pick the argmax.
        ind = np.argmax(qs.data.numpy())
    ...
    def save(self, filepath):
        target_path = filepath+'.qtarget'
        with open(target_path, 'wb') as f:
            torch.save(self.Qtarget, f)
        target_path = filepath+'.vtarget'
        with open(target_path, 'wb') as f:
            torch.save(self.Vtarget, f)

        model_path = filepath+'.qmodel'
        with open(model_path, 'wb') as f:
            torch.save(self.Qpolicy, f)
        model_path = filepath+'.vmodel'
        with open(model_path, 'wb') as f:
            torch.save(self.Vpolicy, f)

    def _load(self, filepath):
        target_path = filepath+'.qtarget'
        with open(target_path, 'rb') as f:
            self.Qtarget = torch.load(f)
        target_path = filepath+'.vtarget'
        with open(target_path, 'rb') as f:
            self.Vtarget = torch.load(f)

        model_path = filepath+'.qmodel'
        with open(model_path, 'rb') as f:
            self.Qpolicy = torch.load(f)
        model_path = filepath+'.vmodel'
        with open(model_path, 'rb') as f:
            self.Vpolicy = torch.load(f)
